...
ssh $TARGET -i $KEY_FILE -t "bash -ci \"cd $HOME_DIR/docker && (docker-compose down easyeaas || true) && (mkdir $TARGET_DEPLOY_DIR || true) && (rm -rf $TARGET_DEPLOY_DIR || true) && (mkdir $TARGET_DEPLOY_DIR || true)\""
